...
import top.whysu.common.utils.HttpUtil;
import top.whysu.front.service.SearchItemService;
import top.whysu.manager.dto.front.EsCount;
...
public class SearchItemServiceImpl implements SearchItemService {
	...
	public int importAllItems() {
		...
		try{
			...
			e.printStackTrace();
			throw new WhysuMallException("导入ES索引库出错，请再次尝试");
		}
	...
	public EsInfo getEsInfo() {
		...
		if(StringUtils.isBlank(healthResult)|| StringUtils.isBlank(countResult)){
			throw new WhysuMallException("连接集群失败，请检查ES运行状态");
		}
		...
		try {
			...
			e.printStackTrace();
			throw new WhysuMallException("获取ES信息出错");
		}
