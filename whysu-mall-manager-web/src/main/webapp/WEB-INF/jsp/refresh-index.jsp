...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 商城管理 <span class="c-gray en">&gt;</span> 同步索引 </nav>
<div style="margin-left: 1vw;margin-right: 1vw" class="cl pd-5 bg-1 bk-gray mt-20">
    <span class="l"> <a class="btn btn-primary radius" onclick="refresh_index()" href="javascript:;"><i class="Hui-iconfont">&#xe645;</i> 一键手动同步索引</a></span>
    <a class="btn btn-secondary radius" style="margin-left: 10px;" href="javascript:location.replace(location.href);" title="刷新"><i class="Hui-iconfont">&#xe68f;</i> 刷新</a>
</div>
<table class="table">
    <tr>
        <td class="va-t">
...
<script type="text/javascript">
    ...
    function refresh_index(){
        layer.confirm('确认要同步数据库中商品数据至ES索引库吗？',{icon:3},function(){
            var index = layer.load(3);
            $.ajax({
                type: 'GET',
                url: '${URL}/item/importIndex',
                success: function(data) {
                    layer.close(index);
                    if(data.success!=true){
                        layer.alert(data.message,{title: '错误信息',icon: 2});
