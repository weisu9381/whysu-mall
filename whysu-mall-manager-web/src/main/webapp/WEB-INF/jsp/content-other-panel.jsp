...
<nav class="breadcrumb">
    <i class="Hui-iconfont">&#xe67f;</i> 首页 &gt; 商城管理 &gt; 其它板块管理
</nav>
<table class="table">
    <tr>
        ...
        <td class="va-t">
            <div class="page-container">
                ...
                <div style="margin-left: 10px;margin-right: 10px" class="cl pd-5 bg-1 bk-gray">
                    <span class="l">
                        <a href="javascript:;" onclick="content_panel_del()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 删除所选板块</a>
                        <a class="btn btn-primary radius" onclick="content_panel_add('添加其它板块','content-other-panel-add',900,500)" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加其它板块</a>
                        <a class="btn btn-secondary radius" href="javascript:location.replace(location.href);" title="刷新"><i class="Hui-iconfont">&#xe68f;</i> 刷新</a>
                    </span>
                </div>
                <%--修改--%>
                <form id="content-panel-edit" action="" method="post" class="form form-horizontal">
                    ...
                    <div class="row cl">
                        <label class="form-label col-xs-3 col-sm-2">position：</label>
                        <div class="formControls col-xs-8 col-sm-9">
                            <input type="text" class="input-text" value="1" name="position" id="position">
                        </div>
                    </div>
                    <div class="row cl">
                        <label class="form-label col-xs-4 col-sm-2">
...
<script type="text/javascript">
        ...
        submitHandler:function(form){
            ...
            $(form).ajaxSubmit({
                ...
                error:function(XMLHttpRequest) {
                    layer.close(index1);
                    layer.alert('修改失败! 错误码:' + XMLHttpRequest.status, {title: '错误信息', icon: 2});
                }
            });
        }
    });
    ...
    /*添加其它板块*/
    function content_panel_add(title,url,w,h){
        layer_show(title,url,w,h)
    }

    /*删除所选板块*/
    function content_panel_del() {
        if(id==-1){
            layer.alert('请选择要删除的板块! ', {title: '错误信息',icon: 0});
            return;
        }
        layer.confirm('确认要删除所选的\''+name+'\'板块吗？',{icon:0},function(){
            var index1 = layer.load(3);
            $.ajax({
                type: 'POST',
                url: '${URL}/panel/del/' +id,
                dataType: 'json',
                success: function(data) {
                    layer.close(index1);
                    if(data.success==true){
